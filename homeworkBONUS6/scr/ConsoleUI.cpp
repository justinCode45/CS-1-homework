...
std::vector<UIObject *>::iterator ConsoleUI::addUIObject(UIObject *uio)
{
    uioj.push_back(uio);
    return uioj.end() - 1;
}
...
vector<vector<UnitChar>> convetStringtoUnitChar(string s)
{
    ...
    variant<BSGR, int> nowColorB = BSGR::black;
...
void decodeColor(string temp, variant<FSGR, int> &resultF, variant<BSGR, int> &resultB)
{
    ...
    while (!ss.eof())
    {
        ...
        case 0:
            ...
            break; 
        default: 
            if ((nowi >= 30 && nowi <= 37) || (nowi >= 90 && nowi <= 97))
            {
                resultF = (FSGR)nowi;
            }
            else if ((nowi >= 40 && nowi <= 47) || (nowi >= 100 && nowi <= 107))
            {
                resultB = (BSGR)nowi;
            }
